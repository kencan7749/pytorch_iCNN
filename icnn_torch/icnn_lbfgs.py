...
import torch
from icnn_torch.utils import  img_deprocess, img_preprocess, normalise_img, \
...
def reconstruct_stim(features, net,
                     ...
                     image_blur=False, sigma_start=2., sigma_end=0.5,
    ...
    if save_intermediate:
        if len(input_size) == 3:
            ...
            if bgr:
                PIL.Image.fromarray(np.uint8(initial_input[..., [2, 1, 0]])).save(
    ...
    for t in range(iter_n):
        ...
        loss = op.step(closure)
        input = input.detach().numpy()[0]


        err = err + loss.detach().numpy()
        loss_list[t] = loss
