...
def reconstruct_stim(features_list, net_list, #[video, image] if video model was used 
                     ...
                     feat_upper_bound = 1.96, feat_lower_bound=-1.96,
    ...
    if  eps_init is None:
        ...
        eps_init = np.random.randn(batch_size, d_E).astype(np.float32)
    if z_C_init is None:
        z_C_init = np.random.randn(batch_size, d_C).astype(np.float32)
        
    #save if save intermediate is True
    if save_intermediate:
        aa = dgn.generate(torch.tensor(eps_init), torch.tensor(z_C_init))
        initial_input = F.interpolate(aa, input_size[1:3]).permute(1,2,3,0).detach().numpy()
        if len(input_size) == 3:
            #image
    ...
    eps_input_ = torch.tensor(eps_init)
    z_C_input_ = torch.tensor(z_C_init)
    ...
    for t in range(iter_n):
        ...
        if opt_name == 'Adam':
            op = optim.Adam([eps_init, z_C_init], lr = lr)
        elif opt_name == 'SGD':
            op = optim.SGD([eps_init, z_C_init], lr=lr, momentum=momentum)
            #op = optim.SGD([input], lr=lr)
        elif opt_name == 'Adadelta':
            op = optim.Adadelta([eps_init, z_C_init],lr = lr)
        elif opt_name == 'Adagrad':
            op = optim.Adagrad([eps_init, z_C_init], lr = lr)
        elif opt_name == 'AdamW':
            op = optim.AdamW([eps_init, z_C_init], lr = lr)
        elif opt_name == 'SparseAdam':
            op = optim.SparseAdam([eps_init, z_C_init], lr = lr)
        elif opt_name == 'Adamax':
            op = optim.Adamax([eps_init, z_C_init], lr = lr)
        elif opt_name == 'ASGD':
            op = optim.ASGD([eps_init, z_C_init], lr = lr)
        elif opt_name == 'RMSprop':
            op = optim.RMSprop([eps_init, z_C_init], lr = lr)
        elif opt_name == 'Rprop':
            op = optim.Rprop([eps_init, z_C_init], lr = lr)

        op.zero_grad()
        gen_v = dgn.generate(eps_init, z_C_init)
        ...
        eps_input_ = eps_init.detach().numpy()
        z_C_input_ = z_C_init.detach().numpy()
        
        
        #clip feat
        if feat_lower_bound is not None:
            eps_input_ = np.maximum(eps_input_ , feat_lower_bound)
            z_C_input_ = np.maximum(z_C_input_ , feat_lower_bound)
        if feat_upper_bound is not None:
            
            eps_input_ = np.minimum(eps_input_ , feat_upper_bound)
            z_C_input_ = np.minimum(z_C_input_ , feat_upper_bound)
